...
import 'users_list_item_table_widget.dart';

class UsersListTableWidget extends StatelessWidget {
  final List<UsersModel> users;

  const UsersListTableWidget({
    super.key,
    required this.users,
  });
  ...
  Widget build(BuildContext context) {
    return Table(
      ...
      children: users
          .map(
            (item) => UsersListItemTableWidget.buildTableRow(
              context: context,
              user: item,
            ),
          )
